$(document).ready(function() {
  ...
  ships["player_one"]["ongoing_damage"]={};
  ships["player_two"]["ongoing_damage"]={};
  ...
  function bind_weapons_and_abilities(){
    ...
    $(".ability_table:not(.not_turn) .weapons td:not(.no_ammo)").on("click", function(){
      ...
      use_weapon(current_player,theweapon,other_player);
      update_health_energy_fill();
    });
    $(".ability_table:not(.not_turn) .abilities td:not(.no_energy)").on("click", function(){
      ...
      {
        attackee = current_player;
      }
      else
      {
        attackee= other_player;
      }
      use_ability(current_player,theability,attackee);
      update_health_energy_fill();
    });
  }

  function use_weapon(attacker, weapon, attackee){
    take_damage(attackee, weapon.damage);
  ...
  {
    take_damage(attackee, ability.damage);
    attacker[0].current_energy -= ability.energy_cost;

    if(ability.turn_length >1)
    {
      if(ability.self_target)
      {
        attacker[0].ongoing_damage[ability.name_value] = {
          "the_ability": ability,
          "turns_left": ability.turn_length
        }
      }
      else
      {
        attackee[0].ongoing_damage[ability.name_value] = {
          "the_ability": ability,
          "turns_left": ability.turn_length
        }
      }
  ...
  function update_ongoing_damage()
  {
    if(!is_empty(current_player[0].ongoing_damage))
    {
      for (var k in current_player[0].ongoing_damage) {
        if (current_player[0].ongoing_damage.hasOwnProperty(k)) 
        {
          take_damage(current_player,current_player[0].ongoing_damage[k].the_ability.damage);
          current_player[0].ongoing_damage[k].turns_left -=1;

          if(current_player[0].ongoing_damage[k].turns_left <= 0)
          {
            delete current_player[0].ongoing_damage[k];
          }
        }
      }
    }
  ...
  function toggle_turns(){
    ...
    if (current_player[1] == "player_one")
    {
    ...
    update_ongoing_damage();

    update_health_energy_fill();
    toggle_buttons();
    bind_weapons_and_abilities();

    if(is_target_dead(current_player[0]))
    {
      end_game();
    }
  }
 ...
 function draw_ship(the_player){
  ...
  function take_damage(target, dam)
  {
    if(target[0].current_health - dam >= target[0].max_health)
    {
      target[0].current_health = target[0].max_health;
    }
    else
    {
      target[0].current_health -= dam;
    }
    ...
    if (dam > 0)
    {
      $("."+target[1]+" #explosion_"+t).fadeIn(400).fadeOut(400);
    }
    else
    {
      $("."+target[1]+" #heal_"+t).fadeIn(400).fadeOut(400);
    }
  }
});
